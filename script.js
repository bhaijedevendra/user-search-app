...
function userRowClicked(id) {
    // alert(`row with id  => ${id} clicked`);
     console.log("previousActiveUser",previousActiveUser);
    ...
    // console.log(activeHighlightedUserDetails);
...
function inputChanged() {
    ...
    var filteredUsers = userData.filter((user, i) => {
        if (user.firstName.toLowerCase().includes(userFirstName.toLowerCase()) ||
            ...
            user.lastName.toLowerCase().includes(userFirstName.toLowerCase()) ||
            user.phone.includes(userFirstName)) {
            return true
        }
    })
